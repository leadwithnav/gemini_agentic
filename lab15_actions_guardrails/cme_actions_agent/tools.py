"""
Action Tools Wrapped with Multi-Layered Defense Middleware (Lab 2).

Integrates Layer 2 OPA (RBAC/ABAC) and Layer 3 Pydantic + Idempotency checks.
"""

from typing import Dict, Any
from .data import PRODUCTS, get_product, update_status_in_db, update_margin_in_db, save_support_ticket
from .policy_engine import defense_policy_engine

# Context helper (Simulated calling user session)
CURRENT_USER_CONTEXT = {
    "username": "jdoe_analyst",
    "role": "SUPPORT_ANALYST"
}

def set_user_context(username: str, role: str) -> None:
    """Set calling user context for testing RBAC/ABAC policies."""
    global CURRENT_USER_CONTEXT
    CURRENT_USER_CONTEXT = {"username": username, "role": role}
...
def update_product_status(
    ...
    """Update market operational trading status (TRADING, HALTED, SUSPENDED, CLOSED)."""
    args = {
        "symbol": symbol,
...
def update_margin_requirement(
    ...
    """Update financial margin requirement (INITIAL, MAINTENANCE) for a CME product."""
    args = {
        "symbol": symbol,
