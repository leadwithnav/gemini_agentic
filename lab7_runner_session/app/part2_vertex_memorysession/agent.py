...
Vertex AI Session Service & Observability

Use Case:
Route CME support requests to specialist agents while:
- resolving conversational follow-up context
- persisting product symbol in Vertex AI Session State (VertexAiSessionService)
...
Request Classifier / Context Resolver
     ...
     ↓
Save Resolved Context in Vertex AI Session State
     ↓
Router
     ↓
 ┌──────────────────┬──────────────────────┬──────────────────┐
...
from ...tools.bigquery_tools import bigquery_toolset, PROJECT_ID, DATASET_ID
from ...callbacks.callback_tools import before_tool_callback, after_tool_callback
